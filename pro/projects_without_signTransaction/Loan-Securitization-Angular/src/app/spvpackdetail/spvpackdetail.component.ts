  ...
  export class SpvpackdetailComponent implements OnInit 
{
  ...
  ngOnInit()
  {
    ...
    this.spv.getUserBalance().then(balance => this.balance = balance)
          ...
          this.details.push({"spvaddress":this.address,"totalamount":result[11],"totalloan":result[1],"availablepack":result[13]});
      })
      ...
      obj.forEach(item => {
          ...
          this.spv.bank_reg(item).then(val=> {
            for(var j=val[3].toNumber();j<val[4].toNumber();j++){
              ...
              {
                this.loandetails.push({"loanid":result[0],"tokenaddress":result[7],"tokenvalue":result[1],"bankaddress":result[2],"totalamount":result[4]});
              }
            })
              })
            }
